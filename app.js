  ...
  visible.forEach((p) => {
    ...
    div.innerHTML = `
      ...
      ${
        p.gram
          ? `
      <button class="addPriceBtn" data-action="add-price" data-id="${p.id}">
        Add by Price
      </button>
      `
          : ""
      ...
      <div class="product-actions">
        <div class="qty-inline">
          ...
          <input type="number" id="qty-${p.id}" value="${
      p.tempQty
    }" min="1" class="qty-input">
          <button class="qty-inc" data-id="${p.id}">+</button>
        </div>
        <button class="addBtn" data-action="select" data-id="${
          p.id
        }">Add</button>
      </div>
    `;
    ...
    div.addEventListener("click", (ev) => {
      //new line
      // if (action === "add-price") return addByPrice(id);
      //--
...
function addByPrice(id) {
  const p = products.find((x) => String(x.id) === String(id));
  if (!p) return;
  if (!p.gram || p.gram <= 0) {
    return alert("This product does not have grams set.");
  }
  const amount = parseFloat(prompt("Enter amount (PKR):"));
  if (isNaN(amount) || amount <= 0) {
    return alert("Enter valid amount.");
  }
  // price per gram
  const perGram = p.price / p.gram;
  // calculate grams for this amount
  const grams = Math.round(amount / perGram);
  // Push to cart as gram-based item
  cart.push({
    name: p.name,
    price: amount, // amount customer is paying
    quantity: grams + " g",
    image: p.image || "",
  });
  saveData();
...
function addProduct() {
  const name = (productNameInput.value || "").trim();
  const price = parseFloat(productPriceInput.value);
  const gram = parseInt(document.getElementById("product-gram").value) || null;
  if (!name || isNaN(price))
    return alert("Enter valid product name and price.");
  const file = productImgInput.files && productImgInput.files[0];
  // Load Image
  if (file) {
    const reader = new FileReader();
    reader.onload = () => {
      ...
      productPriceInput.value = "";
      productImgInput.value = "";
      document.getElementById("product-gram").value = "";
      saveData();
      renderProducts(searchInput.value);
    };
    reader.readAsDataURL(file);
...
function generateBill() {
  ...
  cart.forEach((it) => {
    if (typeof it.quantity === "string" && it.quantity.includes("g")) {
      // Gram-based item → total price already in it.price
      total += Number(it.price);
    } else {
      total += it.price * it.quantity;
    }
  ...
  table.innerHTML = `
    ...
    <tr>
      <td colspan="4" style="text-align:center; padding:4px;">
        Phone: ${(profile.phones || []).join(" | ") || "-"} | Address: ${
    profile.address || "-"
  }
      </td>
    </tr>
  ...
  cart.forEach((it) => {
    ...
    <td style="border:1px solid #ddd; padding:6px; text-align:center;">${qtyDisplay}</td>
    <td style="border:1px solid #ddd; padding:6px; text-align:right;">${it.price.toFixed(
      2
    )}</td>
    <td style="border:1px solid #ddd; padding:6px; text-align:right;">${lineTotal.toFixed(
      2
    )}</td>
  `;
...
function renderHistory() {
  ...
  bills
    ...
    .forEach((b) => {
      ...
      <div style="margin-left:auto">${formatCurrency(
        b.total
      )} <button class="btn" data-action="reprint" data-id="${
        b.id
      }">Reprint</button></div>
    `;
      div.addEventListener("click", (ev) => {
...
function reprintBill(id) {
  ...
  t.innerHTML = `
    <thead>
      <tr><th colspan="4" style="text-align:center">${
        (b.profile && b.profile.shopName) || "Billing Software"
      }</th></tr>
      <tr><th colspan="4" style="text-align:center">Phone: ${(
        (b.profile && b.profile.phones) ||
        []
      ).join(" | ")} | ${(b.profile && b.profile.address) || "-"}</th></tr>
      <tr><th colspan="4" style="text-align:center">ID: ${
        b.id
      } &nbsp;|&nbsp; ${new Date(b.time).toLocaleString()}</th></tr>
  ...
  //   r.innerHTML = `<td>${it.name}</td><td>${it.qty}</td><td>${Number(
  //     it.price
  //   ).toFixed(2)}</td><td>${(it.qty * it.price).toFixed(2)}</td>`;
  //   tbody.appendChild(r);
  // });
  b.items.forEach((it) => {
    let qtyDisplay = it.qty;
    let lineTotal;
  ...
  foot.innerHTML = `<tr><td colspan="3" style="text-align:right">Total:</td><td>${Number(
    b.total
  ).toFixed(2)}</td></tr>`;
  t.appendChild(foot);
...
function renderHeader() {
  ...
  headerSub.innerText = `Phone: ${phones || "-"} | Address: ${
    profile.address || "-"
  }`;
...
function saveProfile() {
  ...
  profile.phones = (phoneInput.value || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  saveData();
...
if (localStorage.getItem(DARK_KEY) === "enabled")
...
if (clearShopBtn)
  clearShopBtn.addEventListener("click", () => clearProfileField("shop"));
if (clearAddressBtn)
  clearAddressBtn.addEventListener("click", () => clearProfileField("address"));
if (clearOfferBtn)
  clearOfferBtn.addEventListener("click", () => clearProfileField("offer"));
if (clearPhoneBtn)
  clearPhoneBtn.addEventListener("click", () => clearProfileField("phones"));
...
if (emojiUpload && productImgInput)
  emojiUpload.addEventListener("click", () => productImgInput.click());
